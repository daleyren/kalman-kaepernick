...
if args.train:
    # series = client.get("/trade-api/v2/series/")['series']
    # filtered_series = [s for s in series if 'KXBTC' in s['ticker']]
    # for s in filtered_series:
    #     print(s)
    
    df=client.get_top_markets(limit=100)
    print(df.head())
    # df = client.get_all_trades("KXBTC")
    # print(df.head())
    # print(df.shape[0])
